...
"""
import torch
from parametric_bandit.experiment import single_rep
from time import time
from torch.distributions import Bernoulli

budget_list = [10, 20, 50, 100, 200]
rep = 10
n_arm = 50  # number of alternatives for each arm
n = [n_arm] * 6
obs_std = 50.0
m = Bernoulli(0.2)
# draw a random number of samples for initializing each arm
num_init_samples = [
    m.sample([n_arm]),
    m.sample([n_arm]),
    m.sample([n_arm]),
...
# TODO: do it so that instead of just running it again and again,
#  we add samples to last one
...
for i in range(len(budget_list)):
    if budget_list[i] not in results.keys():
        ...
        if old_results.shape[0] >= rep:
            continue
        else:
            results[budget_list[i]] = torch.empty((rep, 3))
    for j in range(rep):
        if old_results is not None and j < old_results.shape[0]:
