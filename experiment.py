...
def OCBA_exp(prob: Problem, budget: int, n: list, num_init_samples: int, obs_std: float = None, randomized: bool = False):
    ...
    :return: Optimal arm
    """
    ocba = OCBA(K=sum(n), N=budget, N_0=num_init_samples, randomized=randomized, maximize=True)
...
def composite_exp(prob: Problem, budget: int, n: list, num_init_samples: int, obs_std: float, randomized: bool = False):
    """
